...
class Task(object):
    def __init__(self, env, scenario, sideName):
        ...
        # print(datetime.datetime.fromtimestamp(self.scenario.m_Time))
        self._env = env
        self.sideName = sideName
        self.side = self.scenario.get_side_by_name(self.sideName) # 红方还是蓝方
        ...
        # 输入的可遍历对象生成笛卡尔积，与嵌套的for循环类似
        # a = [1,2,3], b= ['a','b']
        # itertools.product(a,b) --->[(1,a) (1,b), (2,a), (2,b),(3,a), (3,b)]

        # self.time_zone_combine = list(
        #     itertools.product([x for x in range(self.times)], [y for y in range(len(self.defend_zones))]))
        self.time_zone_combine = [y for y in range(len(self.defend_zones))] # 原来为上方
        # 以单个iterable作为参数，并且输入iterable的所有元素也应该是可迭代的，并且他返回包含输入iterable的所有元素的展品的iterable
        # self._actions = [func_list1,func_list2...]
        self._actions = list(
            chain.from_iterable([self._Action('donothing', self._ActionDoNothing, self._DoNothingIsValid),
                                 ...
                                 self._Action('attack', self._AttackAntiSurfaceShipMissionAction,
                                              self._AttackMissionIsValid)])) # ['one', ['two', 'three'], [['four', 'five']]]------>['o', 'n', 'e', 'two', 'three', ['four', 'five']] 即去掉一个中括号
        # 创建一个离散的n维空间，n为整数
        self.action_space = MaskDiscrete(len(self._actions))
        test = len(self._actions)
        print(f"如果{test}是5的话说明我的猜想正确")

    # 这个函数改动去掉了时间
    def _Action(self, type, function, is_valid):
        if type == 'donothing': # 啥也不做 1
            func_list = []
            func_list.append(Function(type=type, function=function(), is_valid=is_valid())) # Function为一个元组
            return func_list
        elif type == 'defensive': # 防御巡逻 30 -> 2
            func_list = []
            for i in self.time_zone_combine:
    ...
    # 返回一个表示当前状态下有效动作的二进制掩码，掩码的维度为71，如果掩码第50个元素为0，则表示动作50此时无效
    ...
    def _DefensiveAirMissionAction(self, missionName, times, zone):
        def act(missionUnit):
            ...
            # 在这里想的是每次调用这个函数的时候获取当前想定中的时间，设置接下来这个missionUnit要做的动作
            scen_time = datetime.datetime.fromtimestamp(self.scenario.m_Time)
            start_time = datetime.datetime.strptime(scen_time.strftime("%m/%d/%Y %H:%M:%S"), '%m/%d/%Y %H:%M:%S')
            mission_time = scen_time + datetime.timedelta(
            ...
            taskParam = {'missionName': missionName, 'missionType': '空战巡逻', 'flightSize': 1, 'checkFlightSize': True,
                         ...
                         'endTime': '%s' % str(mission_time), 'isActive': 'true', 'missionUnit': missionUnit}
