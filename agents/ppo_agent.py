...
class Model(object):
    def __init__(self, *, policy, log_num, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        self.writer = tf.summary.FileWriter("./bin/tfevents_ppo/"+log_num+"/", graph=tf.get_default_graph())


class PPOActor(object):

    def __init__(self, env, side_name, policy, unroll_length, gamma, lam, queue_size=1,
                 enable_push=True, learner_ip="localhost", log_num="0" ,port_A="5700",
                 port_B="5701"):
        self._env = env
...
class PPOLearner(object):
    ...
    def __init__(self, env, policy, unroll_length, lr, clip_range, batch_size,
                 ...
                 unroll_split=8, save_dir=None, log_num="0" ,init_model_path=None,
        ...
        self.writer = SummaryWriter("./bin/tfevents_ppo/"+log_num+"/log/")
    ...
    def run(self):
        ...
        while True:
            ...
            self._model.writer.add_summary(loss_tuple[-1], updates) # 错误？

            self._model_params = self._model.read_params()

            if updates % 10 == 0:
                last_loss = loss[-1]
                episodes = len(self._episode_infos)
                if episodes > 0:
                    reward = self._episode_infos[-1]['r']
                else:
                    reward = 0
                self.writer.add_scalar('loss/policy_loss', last_loss[0], updates)
                self.writer.add_scalar('loss/value_loss', last_loss[1], updates)
                self.writer.add_scalar('reward/reward', reward, episodes)


            if updates % self._print_interval == 0:
                learner_summary = tf.Summary() # 创建摘要
                ...
                learner_summary.value.add(tag="avg_reward", simple_value=avg_reward) # 添加内容
                self._model.writer.add_summary(learner_summary, updates) # 写入
