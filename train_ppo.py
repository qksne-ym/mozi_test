...
os.environ['MOZIPATH'] = 'D:\\software\\Mozi\\MoziServer\\bin'
...
flags.DEFINE_string("save_dir", "./bin/checkpoints/", "Dir to save models to")
...
flags.DEFINE_string("log_num", "0", "log dir num")
...
def start_learner():
    ...
    class Tempenv(object):
        def __init__(self):
            self.action_space = MaskDiscrete(5) # 原来是71维动作
            self.observation_space = spaces.Tuple([spaces.Box(0.0, float('inf'), [740], dtype=np.float32),
                                                   spaces.Box(0.0, 1.0, [5], dtype=np.float32)]) # 原来为71
    ...
    learner = PPOLearner(env=env,
                         ...
                         save_interval=FLAGS.save_interval, # 策略保存频率
                         learn_act_speed_ratio=FLAGS.learn_act_speed_ratio,
                         save_dir=FLAGS.save_dir+"/"+FLAGS.log_num, # 策略保存目录，默认为./checkpoints
                         log_num=FLAGS.log_num,
